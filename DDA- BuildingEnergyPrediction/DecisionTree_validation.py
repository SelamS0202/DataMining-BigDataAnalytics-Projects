...
log_file_name = 'DecisionTree_validation_'+get_log_time()[:10].replace('/','')+'.log'
log_file_name = os.path.join('log', log_file_name)
lf = open(log_file_name, 'w')

#set spark context and spark session
sc = SparkContext()
spark = SparkSession.builder.appName("DecisionTree_validation").getOrCreate()
...
for n in depth_vals:
    ...
    try:
        model = DecisionTree.trainRegressor(train, categoricalFeaturesInfo=categ_feat_info, maxDepth=n, maxBins=32)
    except:
        lf.write('\n\n')
...
with open(os.path.join('results', 'RMSLE_train_DecisionTree.list'), 'wb') as file:
    pickle.dump(rmslq_values_train, file)
with open(os.path.join('results', 'RMSLE_validation_DecisionTree.list'), 'wb') as file:
...
try:
    ...
    plt.legend()
    file_name = os.path.join('plots', 'DecisionTree_validation')
